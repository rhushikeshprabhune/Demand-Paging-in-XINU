...
{	
	...
	kprintf("STARTING MAPPING CREATION FOR XINU PAGES!!!\n");
	uint32 null_pd = get_page_from_paging_area(); //get new page from page table area to be used as pdbr
	system_page_directory = null_pd;
	proctab[currpid].pdbr = system_page_directory;
	map_pages(null_pd, 25);
	write_cr3( null_pd & 0xFFFFF000);
	kprintf("MAPPING FOR XINU PAGES COMPLETE. STARTING PAGING IN 3..2..1 !!!\n");
	//enable paging
	enable_paging();
	kprintf("PAGING SUCESFULLY STARTED!\n");
